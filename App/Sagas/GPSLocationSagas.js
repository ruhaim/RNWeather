...
export function* getLocationResultForGPSLocation(action) {
  ...
  if (!location) {
    yield put(LocationCoordSearchActions.searchCoordLocationFail({ error: 'GPS Error' }));
    return;
  }
  yield put(LocationCoordSearchActions.searchCoordLocationRequest(location.coords));
}
...
export function* searchCoordLocationRequest(api, action) {
  ...
  if (!apiResult.ok) {
    ...
    yield put(LocationCoordSearchActions.searchCoordLocationFail({
      error: { problem, status },
    }));
  }
  yield put(LocationCoordSearchActions.searchCoordLocationSucess(apiResult.data));
}
